...
let rec dump_expr (depth : int) (e : expr) : string =
  ...
  match e with
  | IntLit n -> pad ^ "IntLit(" ^ string_of_int n ^ ")"
  | BoolLit b -> pad ^ "BoolLit(" ^ string_of_bool b ^ ")"
  | Var s -> pad ^ "Var(\"" ^ s ^ "\")"
  | BinOp (op, e1, e2) ->
      pad ^ "BinOp(" ^ string_of_op op ^ ")\n" ^
      dump_expr (depth + 1) e1 ^ "\n" ^
      dump_expr (depth + 1) e2
  | UnaryOp (op, e1) ->
      pad ^ "UnaryOp(" ^ string_of_uop op ^ ")\n" ^
      dump_expr (depth + 1) e1
  | Call (name, args) ->
      pad ^ "Call(\"" ^ name ^ "\")\n" ^
...
let rec inc (key : string) (counts : (string * int) list) : (string * int) list =
  match counts with
  | [] -> [(key, 1)]
  | (k, v) :: rest ->
